...
export const patchProductById = createAsyncThunk(
  ...
  async (value) => {
    console.log([...value.reqobj])
    const response = await axios.patch(`${Api}/products/${value.id}`, value.reqobj
      , { headers: { "Authorization": `Bearer ${token}` } })
    // console.log(response.data)
    return response.data;
  })

const patchProductByIdSlice = createSlice({
...
export default patchProductByIdSlice.reducer;