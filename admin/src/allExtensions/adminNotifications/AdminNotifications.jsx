...
const AdminNotifications = (props) => {
    ...
    useEffect(() => {
        const eventSource = new EventSource(`${Api}/sse?role=admin`); // Connect to SSE endpoint
        //////////////////////
                    ...
                    <div className={`toast${t.visible ? 'animate-enter' : 'animate-leave'} nn`} >
                        <div className="nn2">
                            <div className="nnImg">
                                <img
                                    className="imageCat"
                                    src={`${Api}/users/${eventData.data?.user?.image}`}
                                    alt=""
                                />
                            </div>
                            <div className="nnMessage">
                                <div> {message} {" "} {eventData?.data.user?.username}</div>
                                <div>
    ...
    //////////////////////
    return (
        <div className={props.notificationsListButton ? "notificationsLestT" : "notificationsLestF"} onClick={() => seteng(!eng)} ref={props?.notRef}
            style={{ left: lng == "ar" ? "0px" : "-166px" }}>
            {props?.notHandler(notifications.length)}
            {
                notifications.length > 0 ? (
                    <>
                        {notifications?.slice().reverse().map((order, index) => (
                            <Link to={'/Orders'} className='linkDL newOrder' key={index} onClick={() => props.notificationsListButtonHandler(false)} >
                                <div >
                                    {t('public.newOrder')}
                                </div>
                                <div>
