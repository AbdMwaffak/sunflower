    ...
    <>
      ...
      <div className='bage'>
        <div className='slider'>
          <Carousel
            data-bs-theme='dark'
            style={{ overflow: 'heddin', height: '100%' }}
          >
        ...
        <div className='supTitle'>{t('home.ourCategorys')}</div>
        ...
        <div className='supTitle'>{t('home.ourOffers')}</div>
        <div className='ourOffers'>
          {allOffers?.filter((vv) => {
            if (vv.products.length !== 0) {
              return vv;
            }
          }).length == 0 && (
            <div className='new1'>
          ...
          {allOffers?.filter((vv) => {
            ...
            <>
              <div className='new2'>{t('home.offerOn')}</div>
                ...
                {allOffers
                  ...
                  .map((item, index) => (
                    <CouponCard
                      ...
                      description={
                        lng == 'ar' ? item.descriptionAr : item.description
                      }
        ...
        <div className='supTitle'>{t('home.deliveryServes')}</div>
        <div className='delivery'>
          <div className='new3'>{t('home.deliveryText')}</div>
          <div className='deliveryText'>
            {cities
              ?.filter((cities) => {
                if (cities.isActive == true) {
                  return cities;
                }
              })
              .map((city, index) => (
                <div className='citydev' key={index}>
                  {' '}
                  {lng == 'ar' ? city?.nameAr : city?.name}{' '}
                </div>
              ))}
          </div>
          <div className='map1'>
