...
function LoginModelProdectied(props) {
    ...
    const { data2, loading2, error2 } = useSelector((state) => state.postRegister);
    /////////////////////////
    const goToRegister = () => {
        setWindow(1)
        dispatch(back1());
    ...
    return (
        <>
            ...
            >
                ...
                <Modal.Body  >
                    {error1 && <p className='err' style={{ color: 'red' }}>{error1.message}</p>}
                    {error2 && <p className='err' style={{ color: 'red' }}>{error2.message}</p>}
                    <div style={{ height: window != 1 || data1 == "Verifying..." || data2 == "Verifying..." ? '0px' : "100%" }} className='boxLogin'>
                        <Form noValidate validated={validatedR} onSubmit={handleSubmitRegister}
                        >
                        ...
                        <button className='registerBt' style={{ margin: "0 7%" }} onClick={goBack} >  {t('public.back')}   </button>
                    </div>


                    <div style={{ height: window != 0 || data1 == "Verifying..." || data2 == "Verifying..." ? '0px' : "100%" }} className='boxLogin'>
                        <h3 style={{ width: "100%", textAlign: "center", color: "red" }}>  {t('public.sorry')} !!  </h3>
                        <h5 style={{ width: "100%", textAlign: "center" }}>   {t('public.loginMessage')} </h5>

                        <Form noValidate validated={validatedL} onSubmit={handleSubmitLogin}   >
                            <div className='formBody'>
                                <Form.Group className="mb-3" controlId="form3">
                                    <Form.Label >
                                        {t('public.phone')}
                    ...
                    <div style={{ height: data1 != "Verifying..." && data2 != "Verifying..." ? '0px' : "100%" }} className='boxLogin'>
                        <OTPVerificationForm
                            phone={phone}
                        />
