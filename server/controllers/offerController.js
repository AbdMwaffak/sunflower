...
exports.addProductToOffer = catchAsync(async (req,res,next)=>{
  ...
  await Offer.findByIdAndUpdate(id , 
    { $push: { products: {description , image , name} } }
  );
...
exports.getById = catchAsync(async (req,res,next)=>{
    const {id} = req.params;
    const offer = await Offer.findById(id);
    if(!offer)return next(new AppError('There is no offer with this id🤔' , 404));
...
async function updateProductInOffer(offer, productId, updatedProductData) {
  try {
    ...
    const productIndex = offer.products.findIndex(product => product._id.toString() === productId);
