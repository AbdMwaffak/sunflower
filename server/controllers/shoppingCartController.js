...
exports.add = catchAsync(async (req,res,next)=>{
  const { product, quantity , size , color , paymentMethod} = req.body;
  const userId = req.user.id; 
  // Check If The Basket Contains Flowers
  const itContainsNaturalFlowers = await NaturalFlowerOrder.exists({userId , activeSC : true});
    ...
    if(paymentMethod.toLowerCase() === 'points'){
      const priceInPoints = curProduct.sizes[index].priceInPoints;
      const available = curProduct.sizes[index].available;
      const isAvailableToSellInPoints = curProduct.sizes[index].isAvailableToSellInPoints;
      if(available && isAvailableToSellInPoints && req.user.points >= priceInPoints){
        addProcess = await addProductToCart(priceInPoints ,userId,'pointsProducts',product,size,color,quantity);
      }
    ...
    if(paymentMethod.toLowerCase() === 'money'){
      const price = curProduct.sizes[index].price;
      addProcess = await addProductToCart(price ,userId,'moneyProducts',product,size,color,quantity);
      if(addProcess)return res.status(201).send('Product Added To Shopping Cart 🛒');
    }
...
exports.addOfferToShoppingCart = catchAsync(async(req,res,next)=>{
  ...
  if(cart){
    let itemIndex = cart['offers'].findIndex(p => p.offerId.toString() === offerId.toString());
    if (itemIndex > -1) {
...
exports.deleteOfferFromShoppingCart = catchAsync(async (req,res,next)=>{
  ...
  const cart = await ShoppingCart.findOne({
    userId , 'offers._id' : id
  });
  if(!cart)return next(new AppError('offer not found' , 404));

  await ShoppingCart.findOneAndUpdate({userId} , 
    { $pull: { offers : { _id: id } } }
  );
...
exports.deleteFromSC = catchAsync(async(req,res,next)=>{
  ...
  if(paymentMethod.toLowerCase() === 'money'){
    ...
    cart.moneyProducts = cart.moneyProducts.filter(item => item._id.toString() !== req.params.id);
    cart.modifiedOn =  new Date()
    cart = await cart.save();
    return res.status(201).send('Done!');
  }
  if(paymentMethod.toLowerCase() === 'points'){
    const deletedProduct = cart.pointsProducts.find(p=>p._id.toString() === req.params.id);
    cart.pointsProducts = cart.pointsProducts.filter(item => item._id.toString() !== req.params.id);
    cart.modifiedOn =  new Date()
    cart = await cart.save();
    await User.findByIdAndUpdate(userId , {
...
exports.get = catchAsync(async (req,res,next)=>{
  ...
  if(itContainsNaturalFlowers){
    naturalFlowers = await NaturalFlowerOrder.find({userId , activeSC : true}).populate({
      path : 'naturalFlower',
      select  : '_id -description'
    })
    

    return res.status(200).json({cart : {naturalFlowers , perfumes : [] , products : [] , offers : []}})
  }else{
  ...
  if(!itContainsProducts && !itContainssPerfumes)
    ...
    if(itContainsProducts){
      products = await ShoppingCart.findOne({userId}).populate({
  ...
  return res.status(200).json({
    cart : {
      products : products.moneyProducts?.length  === 0 && products.pointsProducts?.length  === 0? [] : products,
      naturalFlowers : [],
...
exports.emptyTheBasket = catchAsync(async (req,res,next)=>{
  const userId = req.user.id;
  const itContainsNaturalFlowers = await NaturalFlowerOrder.find({userId , activeSC : true});
  if(itContainsNaturalFlowers.length > 0){
    for(let i=0;i<itContainsNaturalFlowers.length;i++){
      await NaturalFlowerOrder.findByIdAndDelete(itContainsNaturalFlowers[i]._id)
  }
...
async function addProductToCart (price ,userId,productsArray , product,size,color,quantity=1){
  
  let cart = await ShoppingCart.findOne({ userId });
  if(cart){
  let itemIndex = cart[productsArray].findIndex(p => p.product.toString() === product.toString() && p.size === size && p.color === color);
    if (itemIndex > -1) {
      ...
      //product does not exists in cart, add new item
      cart[productsArray].push({ product, quantity ,size ,price,color});
      cart.modifiedOn =  new Date()
  ...
  //no cart for user, create new cart
    const newCart = new ShoppingCart();
    newCart.userId = userId;
    newCart[productsArray] = [{ product, quantity , size , price,color }];
    await newCart.save();
